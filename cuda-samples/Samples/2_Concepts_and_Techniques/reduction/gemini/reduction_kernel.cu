// gemini generated kernel
// template <class T>
// __global__ void reduce3(T* data, T* result, unsigned int size) {
//   // Thread index within the block
//   int tid = blockIdx.x * blockDim.x + threadIdx.x;

//   // Handle out-of-bounds threads for non-perfect multiples of block size
//   if (tid >= size) return;

//   // Initialize thread-local sum
//   T partial_sum = data[tid];

//   // Perform reduction using parallel prefix sum algorithm (e.g., Kogge-Stone)
//   // This is a simplified example, specific implementation may vary
//   for (int stride = blockDim.x; stride > 1; stride /= 2) {
//     if (tid < size - stride && tid + stride < size) {
//       partial_sum += data[tid + stride];
//     }
//     __syncthreads();
//   }

//   // Final reduction across all threads (atomic operation needed)
