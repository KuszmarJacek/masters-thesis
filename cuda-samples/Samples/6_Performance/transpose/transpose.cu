...
__global__ void GPT4transposeKernel1(float *odata, float *idata, int size_x, int size_y)
{
    ...
    {
        odata[x * size_y + y] = idata[y * size_x + x];
    }
}

__global__ void GeminitransposeKernel1(float *odata, float *idata, const int size_x, const int size_y) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int j = blockIdx.y * blockDim.y + threadIdx.y;

  // Check for valid indices within matrix bounds
  if (i < size_x && j < size_y) {
    int transposed_idx = j * size_x + i;
    odata[transposed_idx] = idata[(i * size_y) + j];
  }
}

__global__ void GeminitransposeKernel2(float *odata, float *idata, const int size_x, const int size_y) {
  const int block_size = 16; // Adjust based on hardware (multiple of 32 preferred)
  const int shared_size_x = block_size;
  const int shared_size_y = block_size + 1; // Pad for bank conflicts

  __shared__ float shared_data[shared_size_x][shared_size_y];
  ...
  if (i < size_x && j < size_y) {
    ...
    int shared_i = tx;
    int shared_j = ty % block_size;
    shared_data[shared_i][shared_j] = idata[(i * size_y) + j];
    __syncthreads();

    // Access transposed element from shared memory (avoid bank conflicts with padding)
    int transposed_i = shared_j;
    int transposed_j = block_size * blockIdx.y + (shared_i + ty / block_size);
    if (transposed_i < size_x && transposed_j < size_y) {
      odata[transposed_j * size_x + transposed_i] = shared_data[shared_i][shared_j];
    }
  }
}

__global__ void GPT4transposeKernelStrided(float *gold, float *idata, int size_x, int size_y) {
    int x_index = blockIdx.x * TILE_DIM + threadIdx.x;
    int y_index = blockIdx.y * TILE_DIM + threadIdx.y;
    int y_stride = BLOCK_ROWS;

    // Each thread transposes a part of the matrix within the given TILE_DIM
    for (int y = y_index; y < min(y_index + TILE_DIM, size_y); y += y_stride) {
        if (x_index < size_x) {
            gold[y + x_index * size_y] = idata[x_index + y * size_x];
        }
    }
}

__global__ void GPT4transposeWithTiling(float *out, float *in, int width, int height) {
    __shared__ float tile[TILE_DIM][TILE_DIM + 1]; // +1 to avoid shared memory bank conflicts

    int x = blockIdx.x * TILE_DIM + threadIdx.x;
    int y = blockIdx.y * TILE_DIM + threadIdx.y;
    // int widthGrid = gridDim.x * TILE_DIM;
...
__global__ void GPT35transposeKernel1(float *odata, float *idata, const int size_x, const int size_y) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void GPT35transposeKernel2(float *odata, float *idata, const int size_x, const int size_y) {
    __shared__ float tile[TILE_DIM][TILE_DIM + 1];
    ...
    for (int j = 0; j < TILE_DIM; j += BLOCK_ROWS) {
        if (x < size_x && (y + j) < size_y) {
            tile[threadIdx.y + j][threadIdx.x] = idata[(y + j) * size_x + x];
        }
    }
    ...
    for (int j = 0; j < TILE_DIM; j += BLOCK_ROWS) {
        if (x < size_y && (y + j) < size_x) {
            odata[(y + j) * size_y + x] = tile[threadIdx.x][threadIdx.y + j];
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (int k = 0; k < 15; k++)
    {
        ...
        {
        case 0:
            kernel = *GeminitransposeKernel1;
            kernelName = "GeminitransposeKernel1     ";
            break;
        case 13:
            kernel = &copy;
            ...
            break;
        case 1:
            kernel = &GPT4transposeKernel1;
            kernelName = "GPT4transposeKernel1       ";
            break;
        case 8:
        ...
        case 7:
            ...
            break;
        case 9:
            kernel = &GPT4transposeKernelStrided;
            kernelName = "GPT4transposeKernelStrided ";
            break;
        case 10:
            kernel = *GPT4transposeWithTiling;
            kernelName = "GPT4transposeWithTiling    ";
            break;
        case 11:
            kernel = &GPT35transposeKernel1;
            kernelName = "GPT35transposeKernel1      ";
            break;
        case 12:
            kernel = *GPT35transposeKernel2;
            kernelName = "GPT35transposeKernel2      ";
            break;
        case 14:
            kernel = *GeminitransposeKernel2;
            kernelName = "GeminitransposeKernel2     ";
            break;
        }
