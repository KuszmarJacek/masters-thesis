...
#define TILE_SIZE 16
__global__ void GPT35matrix_mult(float *d_a, float *d_b, float *d_result, int m) {
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    ...
    for (int i = 0; i < m / TILE_SIZE; ++i) {
        tile_a[ty][tx] = d_a[row * m + i * TILE_SIZE + tx];
        tile_b[ty][tx] = d_b[(i * TILE_SIZE + ty) * m + col];
        __syncthreads();
...
int main(void)
{
    ...
    GPT35matrix_mult<< < Grid_dim, Block_dim >> > (Left_Vector_d, Right_Vector_d, Res_d, dim);
