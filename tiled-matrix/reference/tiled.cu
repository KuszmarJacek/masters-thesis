...
__global__ void multiply(float *left, float *right, float *res, int dim) {
    ...
    __shared__ float leftBlock [BLOCK_SIZE][BLOCK_SIZE];
    __shared__ float rightBlock[BLOCK_SIZE][BLOCK_SIZE];
    ...
    for (int tileNUM = 0; tileNUM < gridDim.x; tileNUM++) {
        ...
        leftBlock[threadIdx.y][threadIdx.x] = left[row * dim + j];
        rightBlock[threadIdx.y][threadIdx.x] = right[i * dim + col]; 
        __syncthreads();

        for (int k = 0; k < BLOCK_SIZE; k++) {

            temp += leftBlock[threadIdx.y][k] * rightBlock[k][threadIdx.x]; 
        }
        __syncthreads();
    }
